  ...
  return (
    <div className="w-full bg-white flex flex-col">
      ...
      <div className="flex flex-col lg:flex-row border-b-[2px] border-slate-800 bg-white">
        ...
        <div className="flex-[3] border-b-[2px] lg:border-b-0 lg:border-r-[2px] border-slate-800 relative min-h-[400px] flex flex-col">
          ...
          <div className="relative flex-1 w-full p-8 flex justify-center items-center cad-grid">
            <svg viewBox="0 0 1000 300" className="w-full h-full max-h-[300px]" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
              ...
              {/* Deck Line & Bridge */}
              <path d="M 150 80 L 850 80" stroke="#1e293b" strokeWidth="3" fill="none" />
              <path d="M 450 80 L 470 60 L 530 60 L 550 80" stroke="#1e293b" strokeWidth="3" fill="none" />

              {/* Hull Outline */}
              <path d="M 150 100 L 850 100 L 850 140 Q 500 160 150 140 Z" stroke="#1e293b" strokeWidth="3" fill="none" />

              {/* Interactive Highlight Zones (driven by legend hover/lock) */}
              <g className="transition-opacity duration-300">
                <rect x="150" y="100" width="140" height="48" fill="#3b82f6" className={`transition-opacity duration-300 ${activeZone === 1 ? 'opacity-20' : 'opacity-0'}`} />
                <rect x="290" y="100" width="140" height="52" fill="#3b82f6" className={`transition-opacity duration-300 ${activeZone === 2 ? 'opacity-20' : 'opacity-0'}`} />
                <rect x="430" y="100" width="140" height="55" fill="#3b82f6" className={`transition-opacity duration-300 ${activeZone === 3 ? 'opacity-20' : 'opacity-0'}`} />
                <rect x="570" y="100" width="140" height="52" fill="#3b82f6" className={`transition-opacity duration-300 ${activeZone === 4 ? 'opacity-20' : 'opacity-0'}`} />
                <rect x="710" y="100" width="140" height="48" fill="#3b82f6" className={`transition-opacity duration-300 ${activeZone === 5 ? 'opacity-20' : 'opacity-0'}`} />
              </g>
              ...
              {/* Dimension Line */}
              <line x1="150" y1="200" x2="850" y2="200" stroke="#ef4444" strokeWidth="2" markerStart="url(#s03-arrow-red-start)" markerEnd="url(#s03-arrow-red-end)" />
              <rect x="380" y="190" width="240" height="20" fill="#ffffff" />
              <text x="500" y="204" fontSize="12" fontWeight="bold" fill="#ef4444" textAnchor="middle" letterSpacing="1">BELOW-DECK SERVICE LENGTH = 168.0</text>
            </svg>
          </div>
        </div>
            ...
            {turnaroundLogic.map(item => (
              <button
                ...
                className={`flex items-center gap-4 p-4 border-[2px] transition-all cursor-pointer bg-white text-left w-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-slate-800
                  ${activeZone === item.id ? 'border-blue-500 shadow-md scale-105 z-10 text-blue-800' : 'border-slate-800 hover:bg-slate-100 hover:scale-[1.02]'}`}
              >
      ...
      <div className="flex flex-col lg:flex-row bg-white flex-1">
        ...
        <div className="flex-[1] border-b-[2px] lg:border-b-0 lg:border-r-[2px] border-slate-800 flex flex-col min-h-[400px]">
          ...
          <div className="p-8 flex-1 flex justify-center items-center cad-grid">
            <svg viewBox="0 0 600 400" className="w-full h-full max-h-[300px]" aria-hidden="true">
              ...
              <path d="M 100 200 L 250 100 L 450 100 L 500 200 L 450 300 L 250 300 Z" stroke="#1e293b" strokeWidth="3" fill="none" />

              <line x1="100" y1="350" x2="500" y2="350" stroke="#ef4444" strokeWidth="2" markerStart="url(#s03-arrow-red-start-plan)" markerEnd="url(#s03-arrow-red-end-plan)" />
              <rect x="200" y="340" width="200" height="20" fill="#ffffff" />
              <text x="300" y="354" fontSize="12" fontWeight="bold" fill="#ef4444" textAnchor="middle" letterSpacing="1">LOWER PLAN ENVELOPE = 118.0</text>
            </svg>
          </div>
        </div>
        ...
        <div className="flex-[1] flex flex-col min-h-[400px]">
          ...
          <div className="p-8 flex-1 flex justify-center items-center cad-grid">
            <svg viewBox="0 0 600 400" className="w-full h-full max-h-[300px]" aria-hidden="true">
              ...
              <path d="M 200 250 L 200 180 L 250 180 L 270 120 L 330 120 L 350 180 L 400 180 L 400 250 Z" stroke="#1e293b" strokeWidth="3" fill="none" />

              <rect x="180" y="286" width="240" height="20" fill="#ffffff" />
              <text x="300" y="300" fontSize="12" fontWeight="bold" fill="#ef4444" textAnchor="middle" letterSpacing="1">TRANSVERSE SERVICE WIDTH = 42.0</text>
            </svg>
          </div>
        </div>
      </div>
      ...
      {/* FOOTER */}
      <div className="bg-slate-50 border-t-[2px] border-slate-800 p-6 flex justify-between items-end text-[10px] font-bold tracking-widest uppercase relative z-10">
        <div className="flex flex-col items-start gap-2">
          <span className="text-slate-800">20 UNITS</span>
          <div className="w-16 border-t-[3px] border-slate-800" aria-hidden="true"></div>
        </div>
      </div>
