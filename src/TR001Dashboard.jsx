...
import { DoctrineHeader } from './components/Doctrine.jsx';
...
const Eyebrow = ({ as: As = 'p', className = '', children }) => (
  <As className={`text-[10px] font-bold tracking-widest text-[#8F8A82] uppercase ${className}`}>
    {children}
  </As>
);
...
const Sheet06 = ({ lockedPath, setLockedPath }) => {
  ...
  const getBoxStyle = (pathGroup, baseBorder) => {
    const base = `absolute w-[170px] border ${baseBorder} bg-white p-3 z-20 transition-all duration-300 cursor-pointer text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#1A1A1A] `;
  ...
  const getAlertStyle = (pathGroup) => {
    const base = "absolute border border-[#1A1A1A] bg-white p-3 z-20 w-56 transition-all duration-300 cursor-pointer text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#1A1A1A] ";
    if (!currentPath) return base + "opacity-100 scale-100";
    return currentPath === pathGroup
  ...
  return (
    <div className="w-full bg-[#F4F4EE]">
      <DoctrineHeader
        title="TR-001 V0.5 · Trade Architecture"
        subtitle="Propulsion · geometry · climb-rate doctrine"
        chipLeft="SHEET 06 · ARCHITECTURE"
        chipRight="REV A · PROCEDURAL STUDY"
      />

      {/* SECTION 1: OPERATING TRADE MATRIX */}
      <section className="border-b border-[#1A1A1A]">
        <div className="p-4 md:p-6 border-b border-[#1A1A1A] bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-[#1A1A1A] bg-white">
          <div className="p-6 flex flex-col gap-3">
            <div>
              <Eyebrow>STATE A</Eyebrow>
              <h3 className="text-lg tracking-wide text-[#1A1A1A] uppercase mt-1">Installed Power Uplift</h3>
            </div>
            <p className="text-sm text-[#3A3A34] leading-relaxed">
              Test whether more shaft power closes loaded climb and loaded cruise deficits.
            </p>
            <div className="mt-auto pt-6">
              <div className="border border-[#1A1A1A] px-3 py-2 inline-block text-sm text-[#1A1A1A] bg-white mb-2">
                • baseline cap: 11.2 kW total
              </div>
              <p className="text-sm font-semibold text-[#6B6660]">• trade band: 12.0 / 12.5 / 13.0 kW</p>
            </div>
          </div>
          ...
          <div className="p-6 flex flex-col gap-3">
            ...
            <div className="mt-auto pt-6">
              ...
              <p className="text-sm font-semibold text-[#6B6660]">• trade band: -5% / -10% / -15% CdA</p>
            </div>
          </div>
          ...
          <div className="p-6 flex flex-col gap-3">
            ...
            <div className="mt-auto pt-6">
              ...
              <p className="text-sm font-semibold text-[#6B6660]">• trade band: slower departure / lower climb power demand</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-[#1A1A1A] flex flex-col lg:flex-row relative bg-white">
        <div className="flex-1 p-6 md:p-8 relative min-h-[550px] lg:border-r lg:border-[#1A1A1A] overflow-hidden">
          <div className="absolute top-8 right-8 text-[10px] text-[#8F8A82] font-semibold tracking-widest uppercase z-30 pointer-events-none">
          ...
          <div className="relative w-full h-[450px] max-w-[1000px] mx-auto mt-8">
            ...
            <div className={`absolute top-[255px] left-0 text-[11px] font-semibold text-[#6B6660] bg-white pr-2 z-10 transition-opacity ${currentPath && currentPath !== 'core' ? 'opacity-30' : 'opacity-100'}`}>
              baseline rotor...
            </div>
            <div className={`absolute top-[255px] right-0 text-[11px] font-semibold text-[#6B6660] bg-white pl-2 z-10 transition-opacity ${currentPath && currentPath !== 'core' ? 'opacity-30' : 'opacity-100'}`}>
              report candidate...
            </div>
            ...
            <button type="button" aria-pressed={lockedPath === 'core'} aria-label="Sweep A — powertrain cap / core flow path. Toggle highlight." className={getBoxStyle('core', 'border-[#2B5E9D] ring-[#2B5E9D]') + " left-[16%] top-[200px]"} onMouseEnter={handleEnter('core')} onMouseLeave={handleLeave} onClick={handleClick('core')}>
              <p className="text-[9px] font-bold text-[#8F8A82] tracking-wider">SWEEP A / POWERTRAIN CAP</p>
              <h4 className="font-bold text-sm text-[#1A1A1A] leading-tight mt-1">engine power ceiling</h4>
              <div className="mt-8 border border-[#C7C3BC] p-2 text-[10px] text-center font-semibold text-[#3A3A34] bg-white">FRONT ROTOR<br/>54 in disk</div>
              <div className="absolute -right-6 top-[60px] w-12 h-12 rounded-full border border-[#1A1A1A] bg-white flex items-center justify-center text-[9px] font-bold text-center leading-tight">mission-<br/>sim</div>
            </button>
            ...
            <div className={`absolute left-[34%] top-[260px] w-24 text-center z-10 bg-white p-1 transition-opacity ${currentPath ? 'opacity-30' : 'opacity-100'}`}>
              <p className="text-[9px] font-bold text-[#8F8A82]">LOAD / CG BAY</p>
            ...
            <button type="button" aria-pressed={lockedPath === 'drag'} aria-label="Sweep B — airframe drag / CdA reduction path. Toggle highlight." className={getBoxStyle('drag', 'border-[#C0392B] ring-[#C0392B]') + " left-[46%] top-[200px]"} onMouseEnter={handleEnter('drag')} onMouseLeave={handleLeave} onClick={handleClick('drag')}>
              <p className="text-[9px] font-bold text-[#8F8A82] tracking-wider">SWEEP B / AIRFRAME DRAG</p>
              <h4 className="font-bold text-sm text-[#1A1A1A] leading-tight mt-1">CdA reduction cases</h4>
              <p className="text-[9px] mt-2 leading-tight font-semibold text-[#3A3A34]">geometry cleanup applied to cruise model</p>
              <div className="absolute -right-6 top-[100px] w-12 h-12 rounded-sm border border-[#1A1A1A] bg-white flex items-center justify-center text-[9px] font-bold text-center leading-tight">mission-<br/>optimize</div>
            </button>
            ...
            <button type="button" aria-pressed={lockedPath === 'climb'} aria-label="Sweep C — climb demand / departure severity path. Toggle highlight." className={getBoxStyle('climb', 'border-[#D97B29] ring-[#D97B29]') + " left-[73%] top-[200px]"} onMouseEnter={handleEnter('climb')} onMouseLeave={handleLeave} onClick={handleClick('climb')}>
              <p className="text-[9px] font-bold text-[#8F8A82] tracking-wider">SWEEP C / CLIMB DEMAND</p>
              <h4 className="font-bold text-sm text-[#1A1A1A] leading-tight mt-1">departure severity</h4>
              <div className="mt-6 border border-[#C7C3BC] p-2 text-[10px] text-center font-semibold text-[#3A3A34] bg-white">REAR ROTOR<br/>54 in disk</div>
            </button>
            ...
            <button type="button" aria-pressed={lockedPath === 'drag'} aria-label="Current cruise deficit: 12.241 kW peak vs 11.2 kW cap. Toggle drag highlight." className={getAlertStyle('drag') + " right-[10%] top-[30px]"} onMouseEnter={handleEnter('drag')} onMouseLeave={handleLeave} onClick={handleClick('drag')}>
              <p className="text-[9px] uppercase font-bold text-[#8F8A82]">Current Deficit</p>
              <p className="text-xs font-bold text-[#1A1A1A] mt-1">loaded cruise peak: 12.241 kW</p>
              <p className="text-[10px] font-semibold text-[#6B6660] mt-1">vs 11.2 kW installed cap</p>
            </button>
            ...
            <button type="button" aria-pressed={lockedPath === 'climb'} aria-label="Current climb deficit: 11.300 kW peak. Toggle climb highlight." className={getAlertStyle('climb') + " right-[10%] bottom-[30px]"} onMouseEnter={handleEnter('climb')} onMouseLeave={handleLeave} onClick={handleClick('climb')}>
              ...
              <p className="text-[10px] font-semibold text-[#6B6660] mt-1">first failure before cruise relief helps</p>
            </button>

          </div>
        </div>

        <div className="w-full lg:w-72 bg-white p-6 flex flex-col gap-6 border-t lg:border-t-0 border-[#1A1A1A]">
          <div className="border border-[#1A1A1A] bg-white p-4">
            ...
            <ul className="flex flex-col gap-2">
              {['BEMT rotor map artifact', 'OpenVSP drag import', 'mission-sim evaluator', 'mission-optimize search'].map((item, i) => (
                <li key={i} className="border border-[#D5D2CC] p-2 text-[10px] font-semibold text-[#2A2A24] text-center">{item}</li>
              ))}
            </ul>
          </div>

          <div className="border border-[#1A1A1A] bg-white p-4 mt-auto relative z-20">
            <Eyebrow as="h4" className="mb-4">Legend / Gates</Eyebrow>
            <div className="flex flex-col gap-3 text-[11px] font-semibold text-[#3A3A34]" role="group" aria-label="Path highlight toggles">
              <button type="button" aria-pressed={lockedPath === 'core'} className={`flex items-center gap-3 cursor-pointer p-2 -ml-2 transition-colors text-left w-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1A1A1A] ${currentPath === 'core' ? 'bg-[#EDEDE6] text-[#1A1A1A] border border-[#1A1A1A]' : 'hover:bg-[#F4F4EE] border border-transparent'}`} onMouseEnter={handleEnter('core')} onMouseLeave={handleLeave} onClick={handleClick('core')}>
                <div className="w-8 h-[2px] bg-[#2B5E9D]" aria-hidden="true"></div><span>core flow</span>
              </button>
              <button type="button" aria-pressed={lockedPath === 'drag'} className={`flex items-center gap-3 cursor-pointer p-2 -ml-2 transition-colors text-left w-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1A1A1A] ${currentPath === 'drag' ? 'bg-[#EDEDE6] text-[#1A1A1A] border border-[#1A1A1A]' : 'hover:bg-[#F4F4EE] border border-transparent'}`} onMouseEnter={handleEnter('drag')} onMouseLeave={handleLeave} onClick={handleClick('drag')}>
                <div className="w-8 h-[2px] bg-[#C0392B]" aria-hidden="true"></div><span>drag</span>
              </button>
              <button type="button" aria-pressed={lockedPath === 'climb'} className={`flex items-center gap-3 cursor-pointer p-2 -ml-2 transition-colors text-left w-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1A1A1A] ${currentPath === 'climb' ? 'bg-[#EDEDE6] text-[#1A1A1A] border border-[#1A1A1A]' : 'hover:bg-[#F4F4EE] border border-transparent'}`} onMouseEnter={handleEnter('climb')} onMouseLeave={handleLeave} onClick={handleClick('climb')}>
      ...
      <section className="p-6 md:p-8 bg-white relative border-t border-[#1A1A1A]">
        ...
        <div className="relative w-full flex items-center justify-between px-4 md:px-12 mb-8">
          ...
          {[{ top: 'BASELINE', btm: 'current evidence package' }, { top: 'POWER SWEEP', btm: 'can installed cap clear loaded peaks' }, { top: 'DRAG SWEEP', btm: 'can CdA cleanup rescue cruise' }, { top: 'CLIMB SWEEP', btm: 'can departure relief clear takeoff' }, { top: 'TR-001 v0.5', btm: 'publish only if all decision gates pass' }].map((node, i) => (
            <div key={i} className="relative z-10 flex flex-col items-center group cursor-default bg-white px-2">
              ...
              <div className="w-8 h-8 rounded-full border border-[#1A1A1A] bg-white"></div>
              <div className="absolute -bottom-10 text-[10px] font-semibold text-[#3A3A34] text-center w-32 leading-tight">{node.btm}</div>
            </div>
          ))}
        </div>
      </section>
...
const Sheet07 = ({ lockedState, setLockedState }) => {
  ...
  return (
    <div className="w-full bg-[#F4F4EE]">
      <DoctrineHeader
        title="TR-001 V0.5 · Trade Feasibility Surface"
        subtitle="Combined sweep · power cap · drag reduction · climb-rate relief"
        chipLeft="SHEET 07 · FEASIBILITY"
        chipRight="REV A · PROCEDURAL STUDY"
      />
      ...
      <section className="border-b border-[#1A1A1A]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-[#1A1A1A] bg-white">
          <div className="p-6 flex flex-col gap-2">
            ...
            <p className="text-sm font-semibold text-[#6B6660] mt-1">Each case optimized over cruise speed and initial fuel load.</p>
          </div>
          <div className="p-6 flex flex-col gap-2">
            ...
            <p className="text-sm font-semibold text-[#6B6660] mt-1">The infeasible corner is concentrated in the untouched baseline climb scale.</p>
          </div>
          <div className="p-6 flex flex-col gap-2">
            <Eyebrow>Summary C</Eyebrow>
            <h3 className="text-lg text-[#1A1A1A] leading-tight">Best tested case does not require more power</h3>
            <p className="text-sm font-semibold text-[#6B6660] mt-1">At 11.2 kW, geometry plus climb relief can already close the mission.</p>
          </div>
        </div>
      </section>

      {/* GRID DATA & SIDEBAR */}
      <section className="flex flex-col xl:flex-row border-b border-[#1A1A1A] bg-white">
        {/* LEFT: Main Matrix Grid */}
        <div className="flex-[1.5] p-6 md:p-8 border-b xl:border-b-0 xl:border-r border-[#1A1A1A] relative">
                ...
                {row.cells.map((cell, j) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 border flex flex-col items-center justify-center transition-all duration-300 bg-white cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#1A1A1A]
                    ...
                    >
                      <div className={`text-3xl font-light ${isBlocked ? 'text-[#1A1A1A]' : 'text-[#1A1A1A]'}`}>{cell.count}</div>
                      <div className="text-[10px] font-bold text-[#6B6660] mt-1 whitespace-nowrap">peak {cell.peak} kW</div>
                    </button>
                  );
                })}
                   ...
                   {row.annotation && (
                     <div className={`text-xs leading-tight transition-all duration-300 ${currentState && currentState !== row.annotation.type ? 'opacity-30 scale-95' : 'opacity-100 scale-100'}`} style={{ color: row.annotation.color }}>
                        ...
                        <div className="flex items-center gap-2">
                           ...
                           <span className="font-semibold text-[#3A3A34]">{row.annotation.text}</span>
                        </div>
                     </div>
                   )}
                </div>
            ...
            <div className="flex justify-between max-w-sm mx-auto px-4">
              {crossSectionData.map((data, i) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-16 h-24 border flex items-end justify-center pb-2 relative overflow-hidden bg-white ${isBlocked ? 'border-[#C0392B]' : 'border-[#2B5E9D]'}`}>
                    ...
                    <div className="text-[10px] font-semibold text-[#6B6660]">{data.kw} kW</div>
                  </button>
                );
              })}
            </div>
      ...
      <section className="p-6 md:p-8 bg-white relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto flex items-center justify-between pb-12 pt-4 px-4 md:px-12">
           <div className="absolute top-8 left-[10%] w-[25%] -z-10 px-6">
              <div className="relative text-center mb-2 text-[10px] font-bold text-[#6B6660]">first unlock</div>
              <StraightArrowPath color="#C0392B" />
           </div>
           <div className="absolute top-8 left-[38%] w-[25%] -z-10 px-6">
              <div className="relative text-center mb-2 text-[10px] font-bold text-[#6B6660]">expands margin</div>
              <StraightArrowPath color="#D97B29" />
           </div>
           <div className="absolute top-8 right-[10%] w-[25%] -z-10 px-6">
              <div className="relative text-center mb-2 text-[10px] font-bold text-[#6B6660]">clears all tested cells</div>
           ...
           <button type="button" aria-pressed={lockedState === 'blocked'} aria-label="Baseline — blocked corner. Toggle blocked highlight." className={`flex flex-col items-center z-10 w-24 cursor-pointer transition-all bg-white px-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#1A1A1A] ${currentState === 'blocked' ? 'scale-110 font-bold' : 'hover:scale-105'}`} onMouseEnter={handleEnter('blocked')} onMouseLeave={handleLeave} onClick={handleClick('blocked')}>
              ...
              <div className={`w-8 h-8 rounded-full border border-[#1A1A1A] bg-white ${currentState === 'blocked' ? 'ring-4 ring-red-200' : ''}`} aria-hidden="true"></div>
              <div className="text-[10px] font-semibold text-[#3A3A34] text-center mt-2 leading-tight">blocked corner</div>
           </button>
           ...
           <button type="button" aria-pressed={lockedState === 'feasible'} aria-label="Climb relief — first unlock. Toggle feasible highlight." className={`flex flex-col items-center z-10 w-24 cursor-pointer transition-all bg-white px-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#1A1A1A] ${currentState === 'feasible' ? 'scale-110 font-bold' : 'hover:scale-105'}`} onMouseEnter={handleEnter('feasible')} onMouseLeave={handleLeave} onClick={handleClick('feasible')}>
              ...
              <div className={`w-8 h-8 rounded-full border border-[#1A1A1A] bg-white ${currentState === 'feasible' ? 'ring-4 ring-blue-200' : ''}`} aria-hidden="true"></div>
              <div className="text-[10px] font-semibold text-[#3A3A34] text-center mt-2 leading-tight">first unlock</div>
           </button>
